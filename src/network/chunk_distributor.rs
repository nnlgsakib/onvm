...
impl ChunkDistributor {
    ...
    pub async fn fetch_object(&self, object_id: &ObjectId) -> Result<Vec<u8>> {
        ...
        let object = match self.store.get_object_metadata(object_id)? {
            ...
            None => {
                ...
                self.fetch_object_metadata(object_id).await?
            }
        };
        
        self.ensure_providers_for_object(object_id).await;
        ...
        let manifest = match self.store.get_manifest_by_object(object_id)? {
            ...
            None => {
                ...
                self.fetch_manifest(object_id, &object.manifest_id).await?
            }
        };
    ...
    async fn ensure_providers_for_object(&self, object_id: &ObjectId) {
        let has_providers = {
            let provider_map = self.provider_map.read().await;
            provider_map.get(object_id).map(|s| !s.is_empty()).unwrap_or(false)
        };
        
        if !has_providers {
            tracing::info!("no providers in map, querying DHT for object {}", object_id);
            self.network.find_providers(&object_id.0, crate::network::ProviderKind::Program);
            
            for i in 0..5 {
                tokio::time::sleep(Duration::from_secs(1)).await;
                
                let provider_map = self.provider_map.read().await;
                if provider_map.get(object_id).map(|s| !s.is_empty()).unwrap_or(false) {
    ...
    async fn fetch_object_metadata(&self, object_id: &ObjectId) -> Result<crate::types::Object> {
        self.network.find_providers(&object_id.0, crate::network::ProviderKind::Program);
    ...
    async fn fetch_metadata_from_any_peer(&self, object_id: &ObjectId) -> Result<crate::types::Object> {
        ...
        tracing::info!("requesting metadata from {} connected peers via request-response", all_peers.len());
        ...
        let metadata_req_msg = crate::network::unified_protocol::UnifiedProtocolMessage::ObjectMetadataRequest(
    ...
    async fn request_metadata_from_peer(&self, peer: &PeerId, object_id: &ObjectId) -> Result<crate::types::Object> {
    ...
    async fn fetch_manifest(&self, object_id: &ObjectId, _manifest_id: &crate::types::ManifestId) -> Result<crate::types::Manifest> {
        self.network.find_providers(&object_id.0, crate::network::ProviderKind::Program);
    ...
    async fn fetch_manifest_from_any_peer(&self, object_id: &ObjectId) -> Result<crate::types::Manifest> {
        tracing::info!("broadcasting manifest request via gossipsub");
        
        let manifest_req_msg = crate::network::unified_protocol::UnifiedProtocolMessage::ManifestRequest(
    ...
    async fn request_manifest_from_peer(&self, peer: &PeerId, object_id: &ObjectId) -> Result<crate::types::Manifest> {
    ...
    async fn fetch_chunks_loop(&self, object_id: &ObjectId) -> Result<()> {
        ...
        loop {
            ...
            if iterations > max_iterations {
                return Err(anyhow!("chunk fetch timeout after {} iterations", iterations));
            }
    ...
    async fn fetch_single_chunk(&self, object_id: &ObjectId, chunk_id: &ChunkId) -> Result<()> {
        ...
        if providers.is_empty() {
            tracing::debug!("no providers in map for object {}, requesting from any peer", object_id);
            let all_peers: Vec<_> = self.provider_map.read().await
                .values()
    ...
    pub async fn handle_announcement(&self, announcement: ObjectAnnouncement) -> Result<()> {
        ...
        let entry = providers.entry(announcement.object_id).or_insert_with(HashSet::new);
